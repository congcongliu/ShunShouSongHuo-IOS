...
NSString *dateStringWithDateAndFormart(NSString *format,NSDate *date){
    NSTimeZone *timeZone = [NSTimeZone localTimeZone];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setTimeZone:timeZone];
    [formatter setDateFormat:format];
...
NSMutableArray* weekBeginAndEnd(NSString* date)
{
    ...
    BOOL ok = [calendar rangeOfUnit:NSWeekdayCalendarUnit startDate:&beginDate interval:&interval forDate:newDate];
    if (ok) {
        for (int i=0;i<7;i++) {
