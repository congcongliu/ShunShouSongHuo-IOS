...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, SYSTEM_WIDTH, SYSTEM_HEIGHT)])) {
...
+ (void)show {
    [self dismiss:^{
        ...
        [[self instance] timerRun];
    }];
}
- (void)timerRun{
    self.timeOutTimer = [NSTimer scheduledTimerWithTimeInterval:55 target:self selector:@selector(timeoutDissmiss) userInfo:nil repeats:NO];
}
- (void)timerStopRun{
...
+ (void)setGifWithImageName:(NSString *)imageName {
    ...
    [[[self instance] imageView] setCenter:APPDELEGATE.window.center];
}
