...
+ (BOOL)isShow{
    return [[JDStatusBarNotification sharedInstance] isShowTopBar];
}
+ (void)setShow{
    [[JDStatusBarNotification sharedInstance] setIsShowTopBar:NO];
}
...
- (UIView*)showWithStatus:(NSString *)status
    ...
    if (animationsEnabled && style.animationType == JDStatusBarAnimationTypeBounce) {
        ...
        self.isShowTopBar = YES;
        self.topBar.frame = CGRectMake(0, -SYSTEM_STATUS_BAR_HEIGHT, SYSTEM_WIDTH, SYSTEM_STATUS_BAR_HEIGHT);
        [UIView animateWithDuration:(animationsEnabled ? 0.4 : 0.0) animations:^{
            self.topBar.alpha = 1.0;
            self.topBar.transform = CGAffineTransformIdentity;
        } completion:^(BOOL finished) {
            self.topBar.frame = CGRectMake(0, 0, SYSTEM_WIDTH, SYSTEM_STATUS_BAR_HEIGHT);
        }];
    }
    
    if (!self.alphaTimer&&![status containsString:@":"]) {
        self.alphaTimer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(textColorAnnimation) userInfo:nil repeats:YES];
    }
    return self.topBar;
}
...
#pragma mark TextAnnimation

- (void)textColorAnnimation{
    ...
    [UIView animateWithDuration:2 animations:^{
        self.topBar.textLabel.textColor = [UIColor colorWithWhite:1 alpha:alpha];
    }];
}
