...
- (void)playFileWithFilePath:(NSString *)filePath{
    ...
    // 正在播放就返回
    if ([self.player isPlaying]) return;
    self.player = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL URLWithString:filePath] error:NULL];
    self.player.delegate = self;
    [self.session setCategory:AVAudioSessionCategoryPlayback error:nil];
    [self.player play];
}
...
- (AVAudioRecorder *)recorder {
    if (!_recorder) {
        ...
        self.fileName = [NSString stringWithFormat:@"ios_audio_%@_%ld",user_phone(),(long)[NSDate date]];
