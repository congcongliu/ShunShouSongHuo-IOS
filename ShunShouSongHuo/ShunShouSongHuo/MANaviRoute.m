...
/// 补充起点和终点对于路径的空隙
+ (void)replenishFailedPolylinesForStartPoint:(AMapGeoPoint *)start
                               endPoint:(AMapGeoPoint *)end
                              Polylines:(NSMutableArray *)polylines
{
    LineDashPolyline *failedDashPolyline = nil;
    if (start&&end)
    {
        CLLocationCoordinate2D startCoor1 = CLLocationCoordinate2DMake(start.latitude, start.longitude);
        CLLocationCoordinate2D endCoor1 = CLLocationCoordinate2DMake(end.latitude, end.longitude);
        
        MAPolyline *polyline = nil;
        [polyline getCoordinates:&endCoor1 range:NSMakeRange(0, 1)];
        failedDashPolyline = [self replenishPolylineWithStart:startCoor1 end:endCoor1];
    } // end start
    if (failedDashPolyline)
    {
      [polylines addObject:failedDashPolyline];
    }
}
...
+ (instancetype)naviRouteForFailedLinWitheStartPoint:(AMapGeoPoint *)start endPoint:(AMapGeoPoint *)end
{
...
- (instancetype)initWithStartPoint:(AMapGeoPoint *)start endPoint:(AMapGeoPoint *)end
{
    self = [self init];
    ...
    NSMutableArray *polylines = [NSMutableArray array];
    [MANaviRoute replenishFailedPolylinesForStartPoint:start endPoint:end Polylines:polylines];
    self.routePolylines = polylines;
    return self;
}
